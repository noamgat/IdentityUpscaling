...
def save_checkpoint(epoch, epochs_since_improvement, model, metric_fc, race_fc, attr_fc, optimizer, acc, is_best, file_suffix=''):
    ...
    filename = f'checkpoint{file_suffix}.tar'
    torch.save(state, filename)
    # If this checkpoint is the best so far, store a copy so it doesn't get overwritten by a worse checkpoint
    if is_best:
        torch.save(state, f'BEST_checkpoint{file_suffix}.tar')
...
def get_central_face_attributes_img(img):
    bboxes, landmarks = detect_faces(img)
