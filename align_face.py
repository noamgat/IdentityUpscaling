...
parser.add_argument('-split_str', type=str, default='1/1', help='X/Y format for separating the work into splits')
...
def chunkify(lst,n):
    return [lst[idx::n] for idx in range(n)]


if args.split_str != '1/1':
    chunk_idx, num_chunks = [int(part) for part in args.split_str.split('/')]
    all_files = chunkify(all_files, num_chunks)[chunk_idx - 1]
